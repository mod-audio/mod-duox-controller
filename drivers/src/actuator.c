...
//for inverting potentiometer value's 
...
static uint16_t g_pot_value[POTS_COUNT] = {};
...
static void event(void *actuator, uint8_t flags)
{
    ...
    switch (ACTUATOR_TYPE(actuator))
    {
        ...
        case ROTARY_ENCODER:
            ...
            break;
        case POT:
            if (pot->event && (pot->events_flags & flags))
            {
                status = pot->status;
                pot->status &= pot->events_flags;
                pot->event(pot);
                pot->status = status & ~(pot->events_flags & (flags & TRIGGER_FLAGS));
            }
            break;
    }
}
...
void actuator_set_pins(void *actuator, const uint8_t *pins)
{
    ...
    switch (ACTUATOR_TYPE(actuator))
    {
        ...
        case POT:
            ...
            pot->function = pins[2];
            pot->channel = pins[3];
            PINSEL_SetPinFunc(pot->port, pot->pin, pot->function);
            break;
    }
}
...
void actuator_enable_event(void *actuator, uint8_t events_flags)
{
    switch (ACTUATOR_TYPE(actuator))
    {
        ...
        case POT:
            ((pot_t *)actuator)->events_flags = events_flags;
            break;
    }
}
...
uint8_t actuator_get_status(void *actuator)
{
    ...
    switch (ACTUATOR_TYPE(actuator))
    {
        ...
        case POT:
            status = pot->status;
            CLR_FLAG(pot->status, TRIGGER_FLAGS);
            break;
    }
